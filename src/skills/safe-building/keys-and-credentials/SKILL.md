...
- A setting's name, such as APP_API_TOKEN, is not its secret value. Code
  can refer to that name and let the app read the value from its environment
  at runtime. Build this local helper without reading the value.
- Find the name in documentation or a list of setting names. Do not display
  an entire credentials file to learn one name. Use made-up values for
  tests; do not copy a real value into a prompt or a test command.
