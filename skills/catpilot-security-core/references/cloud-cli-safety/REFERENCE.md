...
not shell-split strings. The pure planner `../../scripts/cloud-cli-safety/env_patch.py` preserves
typed values and inverse changes; it never invokes Azure. Its returned
arguments can contain private configuration: use `summarize_patch` for display.
