...
Verification status is in the [table at the end](#verification-status). A channel marked "not verified" has its steps written from the vendor's documentation and has not been exercised by Catpilot; treat it as a plan until someone runs it and records the date.
...
| `install.sh` | Places all of it on one macOS or Linux machine as root, with your server URL substituted; refuses to overwrite a managed file that already exists and prints what to merge instead |
...
The vendor documents a testing variable, `CLAUDE_CODE_MANAGED_SETTINGS_PATH`, that points Claude Code at a managed file outside the system path. In Catpilot's check on 2.1.241 (2026-09-20) it did not take effect: the hooks did not run and the managed server did not load. The system path is the only route this page relies on. Note that the managed file applies to every Claude Code session on the machine, the desktop app included, until `uninstall.sh` removes it.
...
Not yet verified by Catpilot. The steps above are the vendor's documented path.
...
Precedence, highest first: MDM, cloud-delivered requirements from the ChatGPT admin console, the system `requirements.toml`, the user's `config.toml`, command-line overrides. The installed 0.154.0 binary names the `/etc/codex` paths and the MDM keys above. What Catpilot has run is the user-level equivalent: a skill in `~/.agents/skills/` was named and read, and an `mcp_servers.<id>.url` entry in `config.toml` produced a completed `list_approved` call ([verification note](../evals/reports/2026.09.20-org-deployment-verification.md)). The `/etc/codex` paths use the same keys and have not been exercised.
...
| --- | --- | --- | --- |
| Claude Code, managed settings file | not yet verified through the system path; the testing variable `CLAUDE_CODE_MANAGED_SETTINGS_PATH` did not take effect on 2.1.241 ([note](../evals/reports/2026.09.20-org-deployment-verification.md)) | 2026-09-20 | Catpilot |
| Claude Code, managed skills directory | not yet verified | | |
| Claude.ai, organization skills | not yet verified | | |
...
| Codex CLI, `/etc/codex` managed defaults and skills | not yet exercised; the 0.154.0 binary names the paths | 2026-09-20 | Catpilot |
| ChatGPT, workspace plugin | not yet verified; no manifest shipped | | |
| ChatGPT, shared GPT | not yet verified; protocol written | | |
