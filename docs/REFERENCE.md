...
| ChatGPT (project or GPT instructions) | n/a, pasted text; the block was acknowledged in a chat on a Pro account (2026-09-20); the manual protocol in [`evals/HOST_VERIFICATION.md`](../evals/HOST_VERIFICATION.md) is not yet run | custom connector to the hosted endpoint not yet verified | none | | |
