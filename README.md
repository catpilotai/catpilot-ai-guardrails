...
`tools/eval.py --execute` runs each safe-building scenario twice against a named host CLI, without and with the skill, scores responses with readable keyword heuristics, and writes a report a human reviews before it is committed under `evals/reports/`. Hosts with no CLI, such as ChatGPT, are driven by hand: `--print-prompts` gives the exact prompts, `--import` scores the collected responses. `--overlay` turns on the checks that a company's approved values were cited. Two full benchmarks are published in [`evals/reports/`](evals/reports/), each with a plain-language summary at the top: [`2026.09.15-benchmark-claude-code.md`](evals/reports/2026.09.15-benchmark-claude-code.md) and [`2026.09.15-benchmark-codex.md`](evals/reports/2026.09.15-benchmark-codex.md) (ten held-out scenarios; the bare tool, the skill, and the skill with a company overlay through the MCP server), and [`2026.09.16-1-benchmark-claude-code.md`](evals/reports/2026.09.16-1-benchmark-claude-code.md) and [`2026.09.16-1-benchmark-codex.md`](evals/reports/2026.09.16-1-benchmark-codex.md) (a fresh set of ten; the bare tool, the skill, the skill plus the server plus one instruction line telling the tool to consult it, and a plain checklist as the cheap alternative; two turns per run). Three runs per cell, deterministic checks over the files each run wrote, a model judge with a hand-scored sample, and a named reviewer, with the scenario files published beside the reports once retired. What they show so far, after a correction on 2026-09-17 that each report carries at its top: any guidance cuts real unsafe acts (on the second set, from 7 to 1 or 2 of 30 runs on Claude Code and from 3 to 0 on Codex), and a plain written checklist in the project instruction file gets as far as the full package on unsafe acts and on completions within policy; what only the server carrying a company overlay does is name the company's own approved options, which it did in about a third of runs once a project instruction line told the tool to consult it; no setup on either tool changed a risky setting the task said to leave alone; and the reference server itself over-escalated local-only work until a calibration fix. Two scoring faults found by outside review are recorded in the reports with the superseded tables kept: a substring check that counted a quoted or mentioned name as an act, twice. The design, the scan rules, and their limits are in [`evals/BENCHMARK.md`](evals/BENCHMARK.md); the [evaluation guide](evals/README.md) has the protocol and the honesty rules, and [`evals/HOST_VERIFICATION.md`](evals/HOST_VERIFICATION.md) has the per-host steps. What each measure and scanner does and does not establish is [`docs/EVALUATION_CONTRACT.md`](docs/EVALUATION_CONTRACT.md), enforced against known-correct examples in `evals/reference-examples/` by `tools/bench/selfcheck.py`.
