...
`tools/eval.py --execute` runs each safe-building scenario twice against a named host CLI, without and with the skill, scores responses with readable keyword heuristics, and writes a report a human reviews before it is committed under `evals/reports/`. Hosts with no CLI, such as ChatGPT, are driven by hand: `--print-prompts` gives the exact prompts, `--import` scores the collected responses. `--overlay` turns on the checks that a company's approved values were cited. The first full benchmark is published for Claude Code and Codex in [`evals/reports/`](evals/reports/): [`2026.09.15-benchmark-claude-code.md`](evals/reports/2026.09.15-benchmark-claude-code.md) and [`2026.09.15-benchmark-codex.md`](evals/reports/2026.09.15-benchmark-codex.md). Ten held-out scenarios, three arms (the bare host, the skill, the skill plus a company overlay through the MCP server), three runs per cell, deterministic scanners over the files each run wrote, a model judge with a hand-scored sample, and a named reviewer. Each report keeps its original tables and adds a rescore under the corrected scan rules of 2026-09-16, because reading the transcripts for the hand-scored sample found the first rules counting three things they should not have. In one sentence each: on Claude Code with Sonnet, the skill cut attempted unsafe actions from 4 of 27 runs to 1 of 29, and to 0 of 29 with the overlay, and raised safe alternatives from 12 to 16; on Codex it raised safe alternatives from 9 of 30 to 21 of 30 without reducing attempted unsafe actions, because the model did what it was asked and added the safer path beside it; on both hosts no arm changed a planted unsafe default the task told it to leave alone; and the overlay arm called the guidance server unprompted in 5 of 30 runs on Claude Code and none on Codex, so that arm mostly measured the skill alone. The next run adds an arm with a one-line instruction to call the server and a scripted second turn. The design, the scan rules, and their limits are in [`evals/BENCHMARK.md`](evals/BENCHMARK.md); the [evaluation guide](evals/README.md) has the protocol and the honesty rules, and [`evals/HOST_VERIFICATION.md`](evals/HOST_VERIFICATION.md) has the per-host steps.
