...
### `catpilot-safe-building` (bundle `2026.09.18`; `data-in-prompts` and `third-party-services` at `1.0.2`, `hosting-and-where-it-runs`, `sharing-and-publishing`, and `keys-and-credentials` at `1.0.1`, the other three at `1.0.0`)
...
Two fixture sets and one benchmark. The commands below validate the **test inputs**, not agent behavior.
...
`tools/eval.py --execute` runs each safe-building scenario twice against a named host CLI, without and with the skill, scores responses with readable keyword heuristics, and writes a report a human reviews before it is committed under `evals/reports/`. Hosts with no CLI, such as ChatGPT, are driven by hand: `--print-prompts` gives the exact prompts, and `--import` scores the collected responses. `--overlay` checks whether the company's approved values were cited.

Two historical benchmark sets are published in [`evals/reports/`](evals/reports/): [`2026.09.15-benchmark-claude-code.md`](evals/reports/2026.09.15-benchmark-claude-code.md) and [`2026.09.15-benchmark-codex.md`](evals/reports/2026.09.15-benchmark-codex.md), followed by [`2026.09.16-1-benchmark-claude-code.md`](evals/reports/2026.09.16-1-benchmark-claude-code.md) and [`2026.09.16-1-benchmark-codex.md`](evals/reports/2026.09.16-1-benchmark-codex.md). Each set used ten scenarios, three attempts per condition, deterministic file checks, a model judge with a human-scored sample, and a named reviewer. Retired scenario files and original tables remain available. The corrected second-set tables record fewer detected unsafe actions in the guided conditions: 1 or 2 of 30 versus 7 of 30 on Claude Code, and 0 versus 3 on Codex. A generic checklist produced similar counts on that set, while the company-server condition named approved company options in about a third of attempts. The server also unnecessarily escalated local work before a calibration fix. These are descriptive observations from small studies, not proof of a difference, equivalence, or generally secure applications. Historical completion counts used file content, not execution; newer scanner versions do not make those counts functional results. The second set also lacked an explicitly activated skill-only control, so its server comparison does not isolate company rules from the added activation/consultation instruction.

The practical aim is to help builders consult and follow company rules while completing useful work. Fresh confirmation must measure working deliverables, generic safety checks, and policy adherence separately, as well as their combined safe-completion result. The next [experiment plan](docs/EXPERIMENT_PLAN.md) retains the installed-skill deployment question and isolates the company-rules workflow against an explicitly activated skill. It requires independent human task review and non-engineer voice review before the scorer receives fresh tasks.

The design and scanner limits are in [`evals/BENCHMARK.md`](evals/BENCHMARK.md). The [evaluation guide](evals/README.md) has the protocol and honesty rules, and [`evals/HOST_VERIFICATION.md`](evals/HOST_VERIFICATION.md) has the per-host steps. [`docs/EVALUATION_CONTRACT.md`](docs/EVALUATION_CONTRACT.md) defines what each measure establishes; `tools/bench/selfcheck.py` checks those rules against reference examples before candidate runs.
